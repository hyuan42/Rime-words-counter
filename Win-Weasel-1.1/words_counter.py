...
def rime_user_dir() -> Path:
    """返回 Weasel 实际用户目录，兼容安装时设置的自定义 RimeUserDir。"""
    if os.name == "nt":
        try:
            import winreg
...
def default_csv_path() -> Path:
    return rime_user_dir() / "words_input.csv"
...
_DEFAULT_CONFIG_TEMPLATE = """{
  ...
  "csv_path": "",
  "_csv_path_help": "RIME 上屏 CSV 路径。留空 = 自动用 RIME 用户目录根部的 words_input.csv，要和 words_counter.lua 里的 CUSTOM_CSV_PATH 保持一致。",
  "json_path": "",
