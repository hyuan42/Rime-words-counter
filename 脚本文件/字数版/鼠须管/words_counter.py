...
版本: 字数统计工具-鼠须管字数版-1.0
作者: hyuan
Github:https://github.com/hyuan42/Rime-words-counter

脚本功能：处理LUA脚本记录的字数数据（不记录明文，只记录上屏的汉字个数），显示主页面UI，在后台静默运行。
...
CSV_FILE = os.path.join(CUSTOM_PATH2, "words_input.csv")  #记录通过words_counter.lua记录下来的字数的文档，Python脚本通过这个文档统计今日总字数并更新到json中
            ...
            with safe_file_access(JSON_FILE, 'r+') as f:
                ...
                if os.path.exists(CSV_FILE):
                    with safe_file_access(CSV_FILE, 'r') as csv_f:
                        ...
                        try:
                            header = next(reader)
                            if header != ['timestamp', 'chinese_count']:
    ...
    def start_scheduler(self):
        def clear_csv():
            try:
                ...
                self.update_display()
                print(f"[{datetime.now()}] CSV文件已清理")  # 添加日志
            except Exception as e:
                print(f"清理失败: {e}")
