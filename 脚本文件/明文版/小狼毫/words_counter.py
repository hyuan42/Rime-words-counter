...
脚本功能：处理LUA脚本记录的字数+明文数据，统计汇总到json文件，并在系统托盘创建图标、创建一个悬浮窗口实时展示今日字数，同时还具有测速功能、查看历史统计字数的功能等。
            ...
            with safe_file_access(JSON_FILE, 'r+') as f:
                ...
                if os.path.exists(CSV_FILE):
                    with safe_file_access(CSV_FILE, 'r') as csv_f:
                        ...
                        try:
                            if next(reader) != ['timestamp', 'chinese_count', 'text']:
                                return
